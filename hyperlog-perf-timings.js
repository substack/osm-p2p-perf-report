// This is hyperlog v4.10.0 with console.time scattered throughout
// to analyse where time is being spent
...
Hyperlog.prototype.batch = function (docs, opts, cb) {
  ...
  waterfall([
    ...
    function (nodes, done) {
      console.time('8. emit add/reject')
      self.changes = nodes.reduce(maxChange, self.changes)
      console.timeEnd('8. emit add/reject')
      done(null, nodes)
    }
  ], function (err, nodes) {
    console.time('9. unlock')
  ...
  function hashNodesAndFindLinks (nodes, done) {
    var goodNodes = []
    console.time('4. hashNodesAndFindLinks')
    parallel(
      ...
      function (err) {
        console.timeEnd('4. hashNodesAndFindLinks')
        done(err, goodNodes)
      }
    )
  }
  ...
  function lockAndGetSeqNumber (done) {
    console.time('3. lockAndGetSeqNumber')
    ...
    function onlocked (release) {
      self.ready(function () {
        self.logs.head(id, function (err, seq) {
          ...
          console.timeEnd('3. lockAndGetSeqNumber')
  ...
  function computeBatchNodeOperations (nodes, done) {
    console.time('6. computeBatchNodeOperations')
    var batch = []
    var goodNodes = []
    ...
    waterfall(
      ...
      function (err) {
        ...
        console.log('6a. Sign: %sms', signTime)
        console.log('6b. Verify: %sms', verifyTime)
        console.log('6c. Dag: %sms', dagTime)
        console.log('6d. encoder: %sms', encodeTime)
        console.log('6e. batch push: %sms', batchPush)
        console.timeEnd('6. computeBatchNodeOperations')
        console.log('nonClones', nonClones)
        console.log('clones', nodes.length - nonClones)
